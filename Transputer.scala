...
object Transputer {
  def switchStep(transputers: Array[Transputer], stdout: PrintWriter): Unit = {
    ...
    while (i < transputers.length) {
      ...
      while (j < TransputerConstants.IN_PORTS) {
        ...
        if (transputers(i).inputLinks(j).ack eq TransputerConstants.ACKDATA) {
   ...
   */
  private def AtWord(base: Int, nth: Int) = TransputerHelpers.extractWordSelector(base) + nth * TransputerConstants.BYTESPERWORD
...
class Transputer(private val id: Byte, val stdout: PrintWriter, val stderr: PrintWriter) {
  ...
  private def RIndexWord(base: Int, nth: Int) = {
    val bb = ByteBuffer.wrap(mem, Transputer.AtWord(base, nth), TransputerConstants.BYTESPERWORD)
   ...
   */
  private def RIndexByte(base: Int, nth: Int) = mem(Transputer.atByte(base, nth))
  ...
  def printMemory(numberUnits: Int, bytesPerUnit: Int, format: String, address: Int, output: PrintWriter): Unit = {
    output.printf("## Transputer %d\n", id)
    for (unitSelector <- 0 until numberUnits) {
      var unit = 0
      for (byteSelector <- 0 until bytesPerUnit) {
        unit |= (mem(address + (unitSelector * bytesPerUnit) + byteSelector).toLong & 0xFF) << (byteSelector * 8)
      }
  ...
  def printRecentMemory(output: PrintWriter): Unit = {
    ...
    while (i < TransputerConstants.MEMSIZE) {
      if (debuggerState.memAccessed.contains(i) || debuggerState.memAccessed.contains(i + 1) || debuggerState.memAccessed.contains(i + 2) || debuggerState.memAccessed.contains(i + 3)) {
        output.printf("0d%04d/0x%03X    %02X %02X %02X %02X\n", i, i, mem(i + 3), mem(i + 2), mem(i + 1), mem(i))
  ...
  def printWorkspaceMemory(output: PrintWriter): Unit = {
    ...
    for (process <- debuggerState.processes) {
      ...
      while (i <= Transputer.AtWord(wptrs.get(0), 0)) {
        if (wptrs.contains(i) || wptrs.contains(i | 1)) output.printf("-- Workspace %d --\n", wptrs.indexOf(i))
        output.printf("0d%04d/0x%03X    %02X %02X %02X %02X\n", i, i, mem(i + 3), mem(i + 2), mem(i + 1), mem(i))
  ...
  def printLinks(output: PrintWriter): Unit = {
    ...
    i = 0
    while (i < TransputerConstants.IN_PORTS) {
      output.printf("l:%d Wptr:%08X WptrToProcessor:%08X messageLength:%d messagePointer:%08X fromProcessor:%d,toProcessor:%d,hasData:%b,pendingData:%08X,pointer:%08X,count:%08X,inChannel:%08X,readData:%08X,ready:%b,requested:%b,enabled:%b,ack:%d\n", i, inputLinks(i).Wptr, inputLinks(i).WptrToProcessor, inputLinks(i).messageLength, inputLinks(i).messagePointer, inputLinks(i).fromProcessor, inputLinks(i).toProcessor, inputLinks(i).hasData, inputLinks(i).pendingData, inputLinks(i).pointer, inputLinks(i).count, inputLinks(i).inChannel, inputLinks(i).readData, inputLinks(i).ready, inputLinks(i).requested, inputLinks(i).enabled, inputLinks(i).ack)

      i += 1
    }
    output.printf("ol:Wptr:%08X WptrToProcessor:%08X messageLength:%d FptrReg:%08X BptrReg:%08X fromProcessor:%d toProcessor:%d hasData:%b outData:%08X outPointer:%08X outCount:%08X outChannel:%08X outByte:%08X ready:%b requested:%b enabled:%b ack:%b\n", outputLink.Wptr, outputLink.WptrToProcessor, outputLink.messageLength, outputLink.FptrReg, outputLink.BptrReg, outputLink.fromProcessor, outputLink.toProcessor, outputLink.hasData, outputLink.outData, outputLink.outPointer, outputLink.outCount, outputLink.outChannel, outputLink.outByte, outputLink.ready, outputLink.requested, outputLink.enabled, outputLink.ack)
  }
  ...
  private def overflowCheck(opcode: Byte, a: Int, b: Int): Unit = {
    ...
    if (opcode == TransputerConstants.ADD) if (a > 0 && b > TransputerConstants.MAXINT - a) {
      /* handle overflow */
      sreg.errorFlag = true
    }
  ...
  private def blockMoveMiddleStep(): Unit = {
    ...
    if (registers.Areg eq 0) blockMoveFinalStep()
    else {
      ...
      var current = 0
      if (registers.Areg > shift) if (shift == 0) current = RIndexWord(registers.Creg, 0)
  ...
  private def writePartWord(address: Int, word: Int, startByte: Int, len: Int): Unit = {
    var insert = 0
  ...
  private def blockMoveFirstStep(): Unit = {
    ...
    if (registers.Areg eq 0) blockMoveFinalStep()
      ...
      val bytesToWrite = TransputerHelpers.min((TransputerConstants.BITSPERWORD / 8) - DB, registers.Areg)
      var selected = 0
      if (bytesToRead >= bytesToWrite) selected = Transputer.Select(current, current, shift)
      else {
        BMbuffer = current
  ...
  private def enqueueProcess(Wptr: Int, processPriority: Nothing): Unit = {
    ...
    if (FptrReg(processPriorityBit) == TransputerConstants.NOTPROCESS_P) FptrReg(processPriorityBit) = Wptr
    else WIndexWord(BptrReg(processPriorityBit), TransputerConstants.LINK_S, Wptr)
  ...
  private def saveRegisters(): Unit = {
    ...
    if (TransputerHelpers.extractPriority(registers.Wptr) eq LOW) {
      ...
      WIndexWord(TransputerConstants.SAVEBASE, TransputerConstants.STATUSINTSAVE, if (sreg.errorFlag) 1
      else 0)
    }
    // This is not a concurrent simulation so this case will never
    // occur
    if (sreg.moveBit) WIndexWord(TransputerConstants.SAVEBASE, TransputerConstants.EREGINTSAVE, Ereg)
  }
      ...
      case LOW =>
        newProcessPriority match {
          ...
          case LOW =>
            ...
            if (TransputerHelpers.extractWorkspacePointer(registers.Wptr) eq TransputerConstants.NOTPROCESS_P) {
  ...
  private def dequeueProcess(processPriority: Nothing): Unit = {
    ...
    if (FptrReg(processPriorityBit) == BptrReg(processPriorityBit)) FptrReg(processPriorityBit) = TransputerConstants.NOTPROCESS_P
    else FptrReg(processPriorityBit) = RIndexWord(FptrReg(processPriorityBit), TransputerConstants.LINK_S)
  }
  ...
  private def restoreRegisters(): Unit = {
    ...
    if (registers.Wptr ne (TransputerConstants.NOTPROCESS_P | 1)) {
    ...
    if (sreg.moveBit) Ereg = RIndexWord(TransputerConstants.SAVEBASE, TransputerConstants.EREGINTSAVE)
  }
    ...
    currentProcessPriority match {
      case HIGH =>
        ...
        if (FptrReg(0) != TransputerConstants.NOTPROCESS_P) {
          ...
          stdout.printf("NOTPROCESS_P\n")
          restoreRegisters()
          if ((TransputerHelpers.extractWorkspacePointer(registers.Wptr) eq TransputerConstants.NOTPROCESS_P) && FptrReg(1) != TransputerConstants.NOTPROCESS_P) {
            stdout.printf("no interrupted process\n")
            dequeueProcess(LOW)
            activateProcess()
          }
          else if (TransputerHelpers.extractWorkspacePointer(registers.Wptr) eq TransputerConstants.NOTPROCESS_P) {
          }
  ...
  private def enqueueLinkOut(processPointer: Int): Unit = {
    if (outputLink.FptrReg eq TransputerConstants.NOTPROCESS_P) outputLink.FptrReg = processPointer
    else WIndexWord(TransputerHelpers.extractWorkspacePointer(outputLink.BptrReg), TransputerConstants.LINK_S, processPointer)
    outputLink.BptrReg = processPointer
  }

  private def dequeueLinkOut(): Unit = {
    outputLink.WptrPrivate = outputLink.FptrReg
    if (outputLink.FptrReg eq outputLink.BptrReg) outputLink.FptrReg = TransputerConstants.NOTPROCESS_P
    else outputLink.FptrReg = RIndexWord(TransputerHelpers.extractWorkspacePointer(outputLink.FptrReg), TransputerConstants.LINK_S)
  ...
  def processInputLink(inputLink: Nothing): Unit = {
    ...
    if (inputLink.hasData) {
      ...
      if (token == TransputerConstants.ENABLE) inputLink.enabled = true
        ...
        if (inputLink.ready) inputLink.toProcessor = TransputerConstants.READYREQUEST
  ...
  def processOutputLink(): Unit = {
    ...
    if (token == TransputerConstants.PERFORMIO) {
      ...
      if (!outputLink.ready || outputLink.requested || (outputLink.FptrReg ne TransputerConstants.NOTPROCESS_P)) enqueueLinkOut(processPointer)
      else {
    ...
    else if (outputLink.ready && outputLink.requested) if (outputLink.outCount eq 0) {
      outputLink.requested = false
      // PAR
    ...
    else if (outputLink.ready && !outputLink.requested && (outputLink.FptrReg ne TransputerConstants.NOTPROCESS_P)) {
      dequeueLinkOut()
      outputLink.requested = true
    }
  ...
  private def disableChannel(): Unit = {
    if (registers.Breg ne 0 /*FALSE*/ ) {
      ...
      if ((channelNumber & 1) == 0) {
        ...
        if (registers.Breg eq TransputerConstants.NOTPROCESS_P) {
          stdout.printf("Breg == NOTPROCESS_P\n")
          registers.Areg = 0 // FALSE

        }
        else if (registers.Breg eq registers.Wptr) {
        ...
        else if (token == TransputerConstants.READYFALSE) registers.Areg = 0 // FALSE
      }
    }
  ...
  private def enableChannel(): Unit = {
    if (registers.Areg ne 0 /*FALSE*/ ) {
      ...
      if ((channelNumber & 1) == 0) {
        ...
        if (tmp == TransputerConstants.NOTPROCESS_P) WIndexWord(registers.Breg, 0, registers.Wptr)
        else if (tmp == registers.Wptr) {
        }
        else WIndexWord(TransputerHelpers.extractWorkspacePointer(registers.Wptr), TransputerConstants.STATE_S, TransputerConstants.READY_P)
      }
      else { // link-channel
        // TODO: Using physical link
        var token = 0
        ...
        inputLinks(portNumber).toProcessor = TransputerConstants.NOIO
        if (token == TransputerConstants.READYREQUEST) WIndexWord(TransputerHelpers.extractWorkspacePointer(registers.Wptr), TransputerConstants.STATE_S, TransputerConstants.READY_P)
  ...
  private def timerQueueInsertFirstStep(): Unit = {
    ...
    registers.Breg = Transputer.AtWord(TransputerConstants.TIMERBASE, TransputerHelpers.priorityToBit(TransputerHelpers.extractPriority(registers.Wptr)))
  ...
  private def timerQueueInsertMiddleStep(): Unit = {
    ...
    registers.Breg = Transputer.AtWord(registers.Creg, TransputerConstants.TLINK_S)
    timerQueueInsertTest()
  }
  ...
  private def timerQueueInsertTest(): Unit = {
    ...
    if (registers.Creg eq TransputerConstants.NOTPROCESS_P) timerQueueInsertFinalStep()
    else {
  ...
  private def timerQueueInsertFinalStep(): Unit = {
    ...
    registers.Creg = Transputer.AtWord(TransputerConstants.TIMERBASE, TransputerHelpers.extractPriorityBit(registers.Wptr))
    sreg.timeIns = false
    sreg.gotoStartNewProcess = true
  }
  ...
  private def timerQueueDeleteFirstStep(): Unit = {
    ...
    registers.Breg = Transputer.AtWord(TransputerConstants.TIMERBASE, TransputerHelpers.extractPriorityBit(registers.Wptr))
  ...
  private def timerQueueDeleteTest(): Unit = {
    ...
    registers.Creg = RIndexWord(registers.Breg, 0)
    if (registers.Creg eq TransputerHelpers.extractWorkspacePointer(registers.Wptr)) timerQueueDeleteFinalStep()
  }
  ...
  private def timerQueueDeleteFinalStep(): Unit = {
    ...
    if (registers.Breg ne TransputerConstants.NOTPROCESS_P) {
      TNextReg(TransputerHelpers.extractPriorityBit(registers.Wptr)) = RIndexWord(registers.Breg, TransputerConstants.TIME_S)
      TEnabled(TransputerHelpers.extractPriorityBit(registers.Wptr)) = true
    }
    sreg.timeDel = false
  ...
  private def executeSecondaryInstruction(): Unit = {
    ...
    opcode match {
      ...
      case (TransputerConstants.DIV) =>
        stdout.printf("div\n")
        if (((registers.Breg eq TransputerConstants.MININT) && (registers.Areg eq -1)) || (registers.Areg eq 0)) setErrorFlag()
      ...
      case (TransputerConstants.SHL) =>
        ...
        if (shift_val < TransputerConstants.BITSPERWORD) registers.Areg = registers.Breg << registers.Areg
      ...
      case (TransputerConstants.LEND) =>
        ...
        if (registers.Creg > 0) {
          ...
          registers.Creg = registers.Creg + 1
          WIndexWord(registers.Breg, 0, registers.Creg)
          registers.Iptr = Transputer.atByte(registers.Iptr + 1, -registers.Areg)
        }
      ...
      case (TransputerConstants.RET) =>
        ...
        registers.Wptr = Transputer.AtWord(TransputerHelpers.extractWorkspacePointer(registers.Wptr), 4) | TransputerHelpers.extractPriorityBit(registers.Wptr)
        processToUpdate.updateWptr(registers.Wptr)
      ...
      case (TransputerConstants.ALTWT) =>
        ...
        if (registers.Areg eq TransputerConstants.READY_P) {
        }
        else initiateWait()
      ...
      case (TransputerConstants.TIN) =>
        stdout.printf("tin\n")
        laterFlag = Transputer.Later(ClockReg(TransputerHelpers.extractPriorityBit(registers.Wptr)), registers.Areg)
        if (!laterFlag) {
          registers.Areg += 1
      ...
      case (TransputerConstants.TALTWT) =>
        ...
        if (registers.Creg eq TransputerConstants.READY_P) WIndexWord(registers.Wptr, TransputerConstants.TIME_S, ClockReg(TransputerHelpers.extractPriorityBit(registers.Wptr)))
        else {
          registers.Breg = RIndexWord(registers.Wptr, TransputerConstants.TLINK_S)
          if (registers.Breg eq TransputerConstants.TIMENOTSET_P) initiateWait()
          else if (registers.Breg eq TransputerConstants.TIMESET_P) {
      ...
      case (TransputerConstants.ENBT) =>
        ...
        if (registers.Areg ne 0 /*FALSE*/ ) {
          ...
          if (tmp == TransputerConstants.TIMENOTSET_P) {
            ...
            if (laterFlag) WIndexWord(registers.Wptr, TransputerConstants.TIME_S, registers.Breg)
          }
        }
        else if (registers.Areg eq 0 /*FALSE*/ ) {
        }
        registers.Breg = registers.Creg
        registers.Iptr += 1
      ...
      case (TransputerConstants.DIST) =>
        ...
        if (registers.Breg ne 0 /*FALSE*/ ) {
          ...
          if (registers.Oreg eq TransputerConstants.TIMENOTSET_P) registers.Areg = 0 /*FALSE*/
          else if (registers.Oreg eq TransputerConstants.TIMESET_P) {
        ...
        else if (registers.Breg eq 0 /*FALSE*/ ) registers.Areg = 0 // FALSE
        registers.Oreg = 0
      ...
      case (TransputerConstants.TESTERR) =>
        ...
        registers.Creg = registers.Breg
        registers.Breg = registers.Areg
        registers.Areg = if (!sreg.errorFlag) 1 /*TRUE*/
        else 0 /*FALSE*/
      ...
      case (TransputerConstants.TESTHALTERR) =>
        ...
        registers.Creg = registers.Breg
        registers.Breg = registers.Areg
        registers.Areg = if (sreg.haltOnErr) 1 /*TRUE*/
        else 0 /*FALSE*/
        registers.Iptr += 1
  ...
  def unsetBreakpoint(addr: Int, output: PrintWriter): Unit = {
    output.printf("## Transputer %d\n", id)
    if (addr < 0 || addr >= TransputerConstants.MEMSIZE) output.printf("Invalid address\n")
    else if (!debuggerState.breakpoints.contains(addr)) output.printf("No breakpoint found at Hex_addr:%08X Dec_addr:%-8d Value:%08X\n", addr, addr, mem(addr))
    else {
  ...
  def setBreakpoint(addr: Int, output: PrintWriter): Unit = {
    output.printf("## Transputer %d\n", id)
    if (addr < 0 || addr >= TransputerConstants.MEMSIZE) output.printf("Invalid address\n")
    else if (debuggerState.breakpoints.contains(addr)) output.printf("Breakpoint already exists at Hex_addr:%08X Dec_addr:%-8d Value:%08X\n", addr, addr, mem(addr))
    else {
  ...
  private def executePrimaryInstruction(): Unit = {
    ...
    opcode match {
      ...
      case (TransputerConstants.LDLP) =>
        ...
        registers.Creg = registers.Breg
        registers.Breg = registers.Areg
        registers.Areg = Transputer.AtWord(TransputerHelpers.extractWorkspacePointer(registers.Wptr), registers.Oreg)
        registers.Oreg = 0
        registers.Iptr += 1
      ...
      case (TransputerConstants.EQC) =>
        stdout.printf("eqc\n")
        if (registers.Areg eq registers.Oreg) registers.Areg = 1
        else registers.Areg = 0
        registers.Oreg = 0
        registers.Iptr += 1
      ...
      case (TransputerConstants.CJ) =>
        stdout.printf("cj\n")
        if (registers.Areg eq 0) registers.Iptr = Transputer.atByte(registers.Iptr + 1, registers.Oreg)
        else {
          registers.Areg = registers.Breg
          registers.Breg = registers.Creg
          registers.Creg = 0
      ...
      case (TransputerConstants.AJW) =>
        stdout.printf("ajw\n")
        processToUpdate = debuggerState.processes.stream.filter((p) => p.status eq ProcessStatus.RUNNING).filter((p) => p.getCurrentWptr eq registers.Wptr).findFirst.get
        registers.Wptr = Transputer.AtWord(TransputerHelpers.extractWorkspacePointer(registers.Wptr), registers.Oreg) | TransputerHelpers.extractPriorityBit(registers.Wptr)
        registers.Oreg = 0
        registers.Iptr += 1
        processToUpdate.updateWptr(registers.Wptr)
   ...
   */
  private def handleInputChannelRequest(inputLink: Nothing, request: Int): Unit = {
    ...
    if (request == TransputerConstants.RUNREQUEST) {
      ...
      if (status == TransputerConstants.ENABLING_P) WIndexWord(procPtr, TransputerConstants.POINTER_S, TransputerConstants.READY_P)
   ...
   */
  private def handleOutputChannelRequest(outputLink: Nothing, request: Int): Unit = {
    outputLink.toProcessor = TransputerConstants.NOIO
    if (request == TransputerConstants.RUNREQUEST) {
      var channelContent = 0
  ...
  private def checkChannels: Boolean = {
    ...
    i = 0
    while (i < TransputerConstants.IN_PORTS) {
      if (inputLinks(i).toProcessor ne TransputerConstants.NOIO) {
    ...
    if (outputLink.toProcessor ne TransputerConstants.NOIO) {
  ...
  def performStep: Boolean = {
    ...
    val completed = sreg.gotoStartNewProcess || sreg.ioBit || sreg.moveBit || sreg.timeIns || sreg.timeDel
    if (sreg.gotoStartNewProcess) {
      stdout.printf("performStep => startNewProcess\n")
      startNewProcess()
      return true
    }
    else if (TEnabled(0) && (TransputerHelpers.extractPriorityBit(registers.Wptr) eq 0) && completed && Transputer.Later(ClockReg(0), TNextReg(0))) {
    ...
    else if (TEnabled(0) && (TransputerHelpers.extractPriorityBit(registers.Wptr) eq 1) && Transputer.Later(ClockReg(0), TNextReg(0))) {
      stdout.printf("performStep => transputer_handle_timer_reg(0), PRIORITY(1)\n")
      handleTimerRequest(HIGH)
      return true
    }
    else if (TEnabled(1) && (TransputerHelpers.extractPriorityBit(registers.Wptr) eq 1) && completed && Transputer.Later(ClockReg(1), TNextReg(1))) {
  ...
  def logState(index: Long, wr: PrintWriter): Unit = {
    ...
    wr.printf("%10X %08X %08X %08X %08X %08X %08X %01b %01b\n", index, registers.Areg, registers.Breg, registers.Creg, registers.Oreg, registers.Iptr, registers.Wptr, sreg.errorFlag, sreg.haltOnErr)
  }
  ...
  def logSched(index: Long, wr: PrintWriter): Unit = {
    ...
    wr.printf("%08X %08X %08X %08X %08X %01b %01b %01b\n", FptrReg(0), BptrReg(0), FptrReg(1), BptrReg(1), Ereg, sreg.gotoStartNewProcess, sreg.moveBit, sreg.ioBit)
  }
  ...
  def logTimer(index: Long, wr: PrintWriter): Unit = {
    ...
    wr.printf("%08X %08X %08b %08b %01b %01b\n", TNextReg(0), TNextReg(1), TEnabled(0), TEnabled(1), sreg.timeIns, sreg.timeDel)
  }
}