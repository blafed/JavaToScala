...
class Registers(registers: Registers) {
  ...
  var Wptr: Int = registers.Wptr
  var Areg: Int = registers.Areg
  var Breg: Int = registers.Breg
  var Creg: Int = registers.Creg
  var Oreg: Int = registers.Oreg
}
...
//  @throws[UnexpectedOverflowException]
          ...
          case Priority.LOW =>
            ...
            if (
              TransputerHelpers.extractWorkspacePointer(
                registers.Wptr
              ) eq TransputerConstants.NOTPROCESS_P
            ) {
      ...
      case Priority.HIGH =>
        ...
        if (FptrReg(0) != TransputerConstants.NOTPROCESS_P) {
          ...
          if (
            (TransputerHelpers.extractWorkspacePointer(
              registers.Wptr
            ) eq TransputerConstants.NOTPROCESS_P) && FptrReg(
              1
            ) != TransputerConstants.NOTPROCESS_P
          ) {
  ...
  private def enqueueLinkOut(processPointer: Int): Unit = {
    if (outputLink.FptrReg eq TransputerConstants.NOTPROCESS_P)
  ...
  private def dequeueLinkOut(): Unit = {
    ...
    if (outputLink.FptrReg eq outputLink.BptrReg)
      outputLink.FptrReg = TransputerConstants.NOTPROCESS_P
    else
...
//  @throws[UnexpectedOverflowException]
  def processOutputLink(): Unit = {
    ...
    if (token == TransputerConstants.PERFORMIO) {
      ...
      if (
        !outputLink.ready || outputLink.requested || (outputLink.FptrReg ne TransputerConstants.NOTPROCESS_P)
...
//  @throws[UnexpectedOverflowException]
  ...
  private def timerQueueInsertTest(): Unit = {
    ...
    if (registers.Creg eq TransputerConstants.NOTPROCESS_P)
  ...
  private def timerQueueDeleteTest(): Unit = {
    ...
    registers.Creg = RIndexWord(registers.Breg, 0)
    if (
      registers.Creg eq TransputerHelpers.extractWorkspacePointer(
        registers.Wptr
      )
  ...
  private def executeSecondaryInstruction(): Unit = {
    ...
    opcode match {
      ...
      case (TransputerConstants.DIV) =>
        stdout.printf("div\n")
        if (
          ((registers.Breg eq TransputerConstants.MININT) && (registers.Areg eq -1)) || (registers.Areg eq 0)
        ) setErrorFlag()
      ...
      case (TransputerConstants.ALTWT) =>
        ...
        if (registers.Areg eq TransputerConstants.READY_P) {} else
          initiateWait()
      ...
      case (TransputerConstants.TALTWT) =>
        ...
        if (registers.Creg eq TransputerConstants.READY_P)
          WIndexWord(
            registers.Wptr,
      ...
      case (TransputerConstants.DIST) =>
        ...
        if (registers.Breg ne 0 /*FALSE*/ ) {
          registers.Oreg =
            RIndexWord(registers.Wptr, TransputerConstants.TLINK_S)
          if (registers.Oreg eq TransputerConstants.TIMENOTSET_P)
            registers.Areg = 0 /*FALSE*/
...
//  @throws[UnexpectedOverflowException]
  def performStep: Boolean = {
    ...
    if (sreg.gotoStartNewProcess) {
      ...
      TEnabled(0) && (TransputerHelpers.extractPriorityBit(
        registers.Wptr
      ) eq 1) && Transputer.Later(ClockReg(0), TNextReg(0))
    ) {
      stdout.printf(
      ...
      TEnabled(1) && (TransputerHelpers.extractPriorityBit(
        registers.Wptr
      ) eq 1) && completed && Transputer.Later(ClockReg(1), TNextReg(1))
    ) {
      stdout.printf(
